...
var Editor = /** @class */ (function (_super) {
    ...
    Editor.prototype.onSelectionGroupNodeChange = function (node) {
        var newState = this.getEditorState().updateSelectionGroup(node);
    ...
    Editor.prototype.render = function () {
        ...
        return (React.createElement("div", { style: {
            ...
            this.getEditorState().document.nodes.reverse().map(function (node) {
                // Only render the node if it isn't in the selection group or being edited
                if (_this.getEditorState().editing === node.id) {
                    return null;
                }
                if (_this.getEditorState().selectedIDs.count() > 1) {
                    if (_this.getEditorState().selectedIDs.includes(node.id)) {
                        return null;
                    }
                }
            ...
            this.getEditorState().selectionGroup ? (React.createElement(Group, { key: this.getEditorState().selectionGroup.id, node: this.getEditorState().selectionGroup, selected: true, editing: false, onDeselect: function () { return _this.onDeselect(); }, onChange: function (node) { return _this.onSelectionGroupNodeChange(node); } })) : null,
